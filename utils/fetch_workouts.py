...
def populate_leaderboard():
    ...
    try:
        workout_data = fetch_workouts(offline=offline,options=f"[{DB_Enumbs.WEEK}]={this_week}")
...
def fetch_workouts(offline=False,options=None):
    ...
    counted_workouts_df = pd.DataFrame(full_week_workouts_df[DB_Enumbs.NAME].value_counts()).reset_index()
    counted_workouts_df.columns = [DB_Enumbs.NAME,DB_Enumbs.PERFORMED_WORKOUTS]
...
def parse_settings(offline=False):
    data = st.session_state.db_connection.fetch_data_table(
        table_id=DB_Enumbs.USER_SETTINGS_DB,
        offline=offline
    )
    return data

def parse_workouts(data,user_settings):
    df = data.merge(user_settings,on=DB_Enumbs.NAME,how="outer").fillna(0)
    gap = (df[DB_Enumbs.WORKOUT_GOAL] - df[DB_Enumbs.PERFORMED_WORKOUTS]).clip(lower=0)
    df.insert(1,"eligible",df[DB_Enumbs.WORKOUT_GOAL] <= df[DB_Enumbs.PERFORMED_WORKOUTS])
    df.insert(len(df.columns)-1,DB_Enumbs.OWES, gap * df[DB_Enumbs.COST])
    column_names = ["Name","Completed challenge","Workouts Performed","Workout Goal","Missed Workout Cost","Owes","Applied Settings Week"]
    df = df.sort_values(by=["eligible",DB_Enumbs.PERFORMED_WORKOUTS],ascending=False,ignore_index=True)
    df.index = df.index+1
